...
def getWordScore(word, n):
    ...
    sum_score = 0
    for letter in word :
        sum_score += SCRABBLE_LETTER_VALUES.get(letter, 0)
    sum_score *= len(word)
    if n == len(word):
        sum_score += 50
    return sum_score
    ...
    '''
    sum_score = 0    # Statistical total score
    letter_freqence = getFrequencyDict(word)   #get the letter frequence in the word
    for letter in letter_freqence.keys():
        sum_score += SCRABBLE_LETTER_VALUES.get(letter, 0) * letter_freqence.get(letter, 0)
    sum_score *= len(word)
    if n == len(word):
        sum_score += 50
    return sum_score
    ...
    while calculateHandlen(hand) > 0:
        ...
        # Otherwise (the input is not a single period):
            ...
            # Otherwise (the word is valid):
                ...
                this_score = getWordScore(input_word, n)
                total_score += this_score
                print '"%s" earned %d points. Total: %d points' % (input_word, this_score, total_score)
                print
